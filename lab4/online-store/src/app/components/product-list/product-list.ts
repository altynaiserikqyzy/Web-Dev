...
link:'https://kaspi.kz/shop/p/menox-r01-fioletovyi-seryi-135631137/?c=750000000',
  ...
  ngOnInit() {
    // Инициализируем индексы для каждого продукта
  ...
  setMainImage(product: Product, image: string, event: Event) {
 ...
 prevImage(product: Product, event: Event) {
    ...
    this.currentImageIndex[product.id] = newIndex;
    product.image = product.images[newIndex];
  }
  ...
  shareOnWhatsApp(product: any) {
    const text = `Посмотри этот товар: ${product.name} - ${product.link}`;
